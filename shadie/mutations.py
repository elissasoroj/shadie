...
class MutationType:
    ...
    def inspect(self):
        ...
        if self.dist == "n": #distparams= (mean, stddev)
        ...
        source = pd.DataFrame({'counts': dist[0], 'values': dist[1][:-1], 'mean':mean}, columns=['values', 'counts', 'mean'])
        ...
        mean = base.mark_rule(color = alt.ColorName("mediumvioletred")).encode(
            x='mean:Q',
            size=alt.value(3),
            tooltip=['mean:Q']
        )    

...
// shadie function to add 4 recombinant microspores from a 2N genome to p0
...
function (void)sporophyte_selfs(object<Individual>$ ind){
    ...
    for (rep in 1:meiosis_reps);{//for each meiosis rep
        if (runif(1) < 4*GAM_ARCHEGONIA_PER*EGG_SPO_SELF_RATE){//chance megaspoore will self
            ...
            // eggs_selfed = rbinom(1, eggs_made, EGG_SPO_SELF_RATE);
            // number of eggs selfed
            ...
            // choose which eggs self
            selfed_eggs = c();
            for (i in 0:3)
                selfed_eggs = c(selfed_eggs, rbinom(1, megaspore_eggs[i], EGG_SPO_SELF_RATE));
            ...
            for (i in 0:3) {
                ...
                for (egg in 1:(megaspore_eggs[i]-selfed_eggs[i])) {
                    if (i == 0)
                        breaks = breaks1;
                    else if (i == 1)
                        breaks = breaks2;
                ...
                // for each egg they make
                for (egg in 1:selfed_eggs[i]) {
                    if (i<2) {
                        if (rbinom(1, 1, 0.5) > 0)
                            fbreaks = breaks1;
                        else
            ...
            // reps of meiosis that still need to be performed
            additional_reps = 1 + additional_reps;
    }
